...
import numpy as np
from numpy import zeros, floor, arange
import pylab as p
...
x = zeros(Nn + 1, float)
y = zeros(Nn + 1, float)
Ebcnod = zeros(Nn + 1, int)
Ebcval = zeros(Nn + 1, int)
node = zeros((Ne + 1, 4), int)
...
# Evaluate element stiffness matrix Ke and element vector ge

    Ke[1, 1] = -(y23 * y23 + x32 * x32) / (2 * J)
    Ke[1, 2] = -(y23 * y31 + x32 * x13) / (2 * J)
    Ke[2, 1] = Ke[1, 2]
    Ke[1, 3] = -(y23 * y12 + x32 * x21) / (2 * J)
    Ke[3, 1] = Ke[1, 3]
    Ke[2, 2] = -(y31 * y31 + x13 * x13) / (2 * J)
    Ke[2, 3] = -(y31 * y12 + x13 * x21) / (2 * J)
    Ke[3, 2] = Ke[2, 3]
    Ke[3, 3] = -(y12 * y12 + x21 * x21) / (2 * J)
    ge[1] = 0
    ge[2] = 0
    ge[3] = 0

# Assemble element stiffness into global A matrix
...
for ii in range(1, 11):
    for jj in range(1, 11):
        for e in range(1, Ne):
            x2p = x[node[e, 2]] - X[ii, jj]
            x3p = x[node[e, 3]] - X[ii, jj]
            y2p = y[node[e, 2]] - Y[ii, jj]
            y3p = y[node[e, 3]] - Y[ii, jj]
            A1 = 0.5 * abs(x2p * y3p - x3p * y2p)
            x1p = x[node[e, 1]] - X[ii, jj]
            y1p = y[node[e, 1]] - Y[ii, jj]
            A2 = 0.5 * abs((x[node[e, 2]] - X[ii, jj]) * y1p - x1p * (y[node[e, 2]] - Y[ii, jj]))
            ...
            if J == 0:
                continue
            if abs(J / 2 - (A1 + A2 + A3)) < 0.00001 * abs(J / 2):
                ksi = (y31 * (X[ii, jj] - x[node[e, 1]]) - x31 * (Y[ii, jj]
                       - y[node[e, 1]])) / J
                ita = (-y21 * (X[ii, jj] - x[node[e, 1]]) + x21 * (Y[ii, jj]
                       - y[node[e, 1]])) / J
                N1 = 1 - ksi - ita
                N2 = ksi
                N3 = ita
                Vgrid[ii, jj] = N1 * V[node[e, 1]] + N2 * V[node[e, 2]] \
                    + N3 * V[node[e, 3]]
                break
